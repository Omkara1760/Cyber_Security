...
//		here we are doing matrix multiplication for each s00,s10,s01,s11
		...
		S00=exor(multiplyReduceTo4(_1,Me_[0][0]),multiplyReduceTo4(_2,Me_[0][1]));
		S10=exor(multiplyReduceTo4(_1,Me_[1][0]),multiplyReduceTo4(_2,Me_[1][1]));
		S01=exor(multiplyReduceTo4(_3,Me_[0][0]),multiplyReduceTo4(_4,Me_[0][1]));
		S11=exor(multiplyReduceTo4(_3,Me_[1][0]),multiplyReduceTo4(_4,Me_[1][1]));
...
//	and returns this 4 bits as array
	...
	int [] multiplyReduceTo4(int []ar,int n)
	{
		...
		int []reduce4= {1,0,0,1,1};	//this is polynomial irreducible equation for degree 4 
...
//		flag is 1 for reverse substitution
	...
	public static void main(String[] args) {			
		...
		obj.keyGeneration();                          // function which generates 3 key  
		
		int [] plaintext= {1,1,0,1,0,1,1,1,0,0,1,0,1,0,0,0};     //hard coded plain text
		
		System.out.println();
		System.out.println("Your plain Text is:");
		obj.printarray(plaintext);								//printing the plain text
		
		System.out.println();
		int []ciphertext=obj.encryption_(plaintext);    // here we are doing encryption 
		System.out.println("Your Cipher Text Is:");
		obj.printarray(ciphertext);						//printing the cipher text
		
		System.out.println();
		obj.decryption(ciphertext);						//here we are doing decryption by using ciphertext
